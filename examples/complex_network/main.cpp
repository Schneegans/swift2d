...
#include "../../third_party/raknet/src/Rand.h"
...
#include "../../third_party/raknet/src/TeamManager.h"
...
#include "../../third_party/raknet/src/ReadyEvent.h"
#include "../../third_party/raknet/src/PacketLogger.h"
#include "../../third_party/raknet/src/RPC4Plugin.h"
// #include "../../third_party/raknet/src/Kbhit.h"
	...
	while (game->phase!=Game::EXIT_SAMPLE)
	{
		...
		if (httpConnection2->GetResponse(stringTransmitted, hostTransmitted, responseReceived, hostReceived, contentOffset))
		{
			if (responseReceived.IsEmpty()==false)
			{
				...
				{
					// No content
					printf("%s", responseReceived.C_String());
				}
				else
				{
					json_error_t error;
						...
						while (iter)
						{
							...
							if (strcasecmp(firstKey, "GET")==0)
							{
								...
								for (unsigned int i=0; i < arraySize; i++)
								{
									...
									printf("Room name: %s. Players: %i\n", json_string_value(roomNameVal), (int)natTypesSize);
								}
